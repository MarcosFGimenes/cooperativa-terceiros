...
describe("package Excel export", () => {
  it("exports one row per service in the requested column order", () => {
    const workbook = buildPackageServicesExcel([
      { os: "OS-10", tag: "TAG-2", equipment: "Bomba", progress: 42.5, company: "Empresa A" },
      { os: "OS-11", tag: "TAG-3", equipment: "Motor", progress: 100, company: "Empresa B" },
    ]);
  ...
  it("escapes spreadsheet content and creates a safe filename", () => {
    const workbook = buildPackageServicesExcel([
      { os: "A&B", tag: "<tag>", equipment: 'Equipamento "1"', progress: 0, company: "A > B" },
    ]);
